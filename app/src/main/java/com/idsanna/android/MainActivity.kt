...
class MainActivity : Activity() {
    ...
    private fun render() {
        ...
        val code = TextView(this).apply { text = "Código de activación: ${session.activationCode()}"; textSize = 16f; setPadding(0,0,0,24) }
        ...
        root.addView(title); root.addView(status); root.addView(code); root.addView(start); root.addView(listen); root.addView(stop); root.addView(accessibility); root.addView(overlay); setContentView(root)
    }
