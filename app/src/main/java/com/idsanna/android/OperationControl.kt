...
class OperationControl(
    ...
    fun start(
        ...
        if (operations.containsKey(operationId) || persistence?.get(operationId) != null) return false
        ...
        val record = OperationRecord(
            ...
            toolName = toolName,
            startedAt = now,
            timeoutAt = now + timeoutMs.coerceIn(MIN_TIMEOUT_MS, MAX_TIMEOUT_MS),
    ...
    fun complete(operationId: String, evidence: String? = null): Boolean = transition(operationId, OperationState.COMPLETED, evidence)
    fun fail(operationId: String, evidence: String? = null): Boolean = transition(operationId, OperationState.FAILED, evidence)

    fun isExpired(operationId: String): Boolean {
        val record = operations[operationId] ?: return false
        if (record.state != OperationState.RUNNING || clock.nowMillis() < record.timeoutAt) return false
        operations.computeIfPresent(operationId) { _, current ->
            if (current.state == OperationState.RUNNING) current.copy(state = OperationState.TIMED_OUT, evidence = "timeout") else current
        }
    ...
    fun record(operationId: String): OperationRecord? = operations[operationId] ?: persistence?.get(operationId)
    ...
    private fun transition(operationId: String, next: OperationState, evidence: String?): Boolean {
        val updated = operations.computeIfPresent(operationId) { _, current ->
            if (current.state == OperationState.RUNNING) current.copy(state = next, evidence = evidence) else current
        }
