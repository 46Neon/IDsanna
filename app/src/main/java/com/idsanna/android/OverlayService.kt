...
class OverlayService : Service() {
    ...
    private fun showPanel() {
        ...
        val send = Button(this).apply { text = "Enviar"; setOnClickListener { val command = input.text.toString().trim(); if (command.isNotEmpty()) { val task = TaskStore(this@OverlayService).enqueue(command); taskStatus.text = "Tarea creada\nID: ${task}\nEstado: queued\nInstrucción guardada: $command"; Toast.makeText(this@OverlayService, "Tarea guardada", Toast.LENGTH_SHORT).show(); input.text.clear() } } }
