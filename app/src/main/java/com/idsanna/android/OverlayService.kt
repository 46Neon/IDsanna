...
class OverlayService : Service() {
    ...
    private fun showPanel() {
        ...
        val send = Button(this).apply { text = "Enviar"; setOnClickListener { val command = input.text.toString().trim(); if (command.isNotEmpty()) { val taskStore = TaskStore(this@OverlayService); val task = taskStore.enqueue(command); val parsed = InstructionParser().parse(command); val plan = Planner(ToolRegistry.defaults()).createPlan(task, parsed); val step = plan.steps.firstOrNull(); val status = when { !parsed.valid || plan.errors.isNotEmpty() -> "failed"; step?.tool == "cad.create_geometry" || step?.tool == "termux.run_registered_task" -> "approval_required"; else -> "planned" }; taskStore.updateStatus(task, status); val readable = when (status) { "approval_required" -> "aprobación requerida"; "planned" -> "planificada"; "failed" -> "no planificable"; else -> status }; taskStatus.text = "Tarea registrada\nID: ${task}\nEstado: ${readable}\nIntención: ${parsed.intent}\nDestino: ${parsed.target}\nPlan: ${step?.tool ?: "sin herramienta"}\nEjecución: aún no iniciada\nVerificación: pendiente\n\nInstrucción guardada:\n$command"; Toast.makeText(this@OverlayService, "Tarea guardada", Toast.LENGTH_SHORT).show(); input.text.clear() } } }
